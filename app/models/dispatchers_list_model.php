<?php
session_start();
  class Dispatchers_List_Model extends Model {
   public function __construct($lang = 'ua', $city = 'te') {
    parent::__construct();
    //pre init
    $this->data['current_page'] = "dispatchers/dispatchers_sub_all";
    ...
    $sql_dispatchers = "SELECT * FROM dispatchercard_view ORDER BY name ASC LIMIT 32";
    //prepare globals
    ...
    //prepare drivers
    $stm = $pdo->prepare($sql_dispatchers);
    //statement drivers
    $stm->execute();
    //fetching drivers array
    while ($db_data = $stm->fetch(PDO::FETCH_ASSOC)){
      $this->data['dispatchers'][] = $db_data;    
    }
   }
  }
?>